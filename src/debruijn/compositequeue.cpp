...
mem_b CompositeQueue::high_prio_flowsize = 0;
  ...
  : Queue(bitrate, maxsize, eventlist, logger)
{
  ...
  _queuesize_low_per_destination.resize(num_tors);
  _queuesize_high_per_destination.resize(num_tors);

  _queuesize_low_by_origin.resize(2);
  _queuesize_high_by_origin.resize(2);

  data_bytes_received_at_dst_tor = 0;
  header_bytes_received_at_dst_tor = 0;
}
...
void CompositeQueue::completeService() {
    ...
    if (_serv == QUEUE_LOW) {
  	...
  	if (!pipe_is_down && (!_enqueued_high.empty() || !_enqueued_low.empty() || !_enqueued_rlb.empty()))
  		beginService();
}
	...
	switch (pkt.type()) {
                ...
                if (_queuesize_low + pkt.size() <= _maxsize || drand()<0.5) {
                    ...
                    if (_queuesize_low + pkt.size() > _maxsize) {
                        ...
                        if (booted_pkt->size() >= pkt.size()) {
                            ...
                            _queuesize_low_per_destination[booted_pkt->get_dst_ToR()] -= booted_pkt->size();

                            if (booted_pkt->get_src_ToR() == booted_pkt->get_crtToR()) {
                                _queuesize_low_by_origin[0] += booted_pkt->size();
                            } else {
                                _queuesize_low_by_origin[1] += booted_pkt->size();
                            }
        ...
        case TCP: {
            ...
            if (tcp_pkt->get_tcp_src()->get_flowsize() >= high_prio_flowsize) {
                ...
                // Small flow put to high prio queue
                if (_queuesize_high + pkt.size() > _maxsize) {
                    // debug:
                    // cout << "   Queue full. Dropped.\n";
                    //if (_logger) _logger->logQueue(*this, QueueLogger::PKT_DROP, pkt);
                ...
                // the new packet fit
                assert(_queuesize_high + pkt.size() <= _maxsize);
                _enqueued_high.push_front(&pkt);
                _queuesize_high += pkt.size();
                _queuesize_high_per_destination[pkt.get_dst_ToR()] += pkt.size();
...
mem_b CompositeQueue::queuesize_high() {
    return _queuesize_high;
}

mem_b CompositeQueue::queuesize_low() {
    return _queuesize_low;
}

void CompositeQueue::strip_packets_after_da_disconnect() {
    if (_enqueued_low.empty()) return;

    for (int pkt_idx = 0; pkt_idx < _enqueued_low.size(); pkt_idx++) {
        // Do not track stripping here since this is not due to queue overflow.
        // _bytes_stripped += pkt.size() - 64;
        // _num_stripped++;
        //strip payload on the arriving packet - low priority queue is full
        Packet* pkt = _enqueued_low.back();
        _enqueued_low.pop_back();
        if (pkt_idx == 0 && _serv == QUEUE_LOW) {
            _enqueued_low.push_front(pkt);
            continue;  // Do not strip first packet since it is currently in service
        }
        _queuesize_low -= pkt->size();
        _queuesize_low_per_destination[pkt->get_dst_ToR()] -= pkt->size();
...
void CompositeQueue::clear_queue() {
    _serv = QUEUE_INVALID;
    ...
    if (!_enqueued_low.empty()) {
        ...
        _queuesize_high_per_destination.assign(_queuesize_low_per_destination.size(), 0);
...
mem_b CompositeQueue::queuesize_per_destination(uint32_t tor_id) {
    return _queuesize_low_per_destination[tor_id] + _queuesize_high_per_destination[tor_id];
}

mem_b CompositeQueue::queuesize_local_traffic() {
    return _queuesize_low_by_origin[0] + _queuesize_high_by_origin[0];
}

mem_b CompositeQueue::queuesize_nonlocal_traffic() {
    return _queuesize_low_by_origin[1] + _queuesize_high_by_origin[1];
}

void CompositeQueue::notify_connect_da_link() {
    if (_serv==QUEUE_INVALID) {
        // No other packet is being drained
        if (queuesize() > 0) {
            beginService();
        }
    }
...
mem_b CompositeQueue::report_bytes_at_dst() {
    mem_b temp;
    temp = data_bytes_received_at_dst_tor;
    data_bytes_received_at_dst_tor = 0; // reset the counter
    return temp;
}

mem_b CompositeQueue::report_header_bytes_at_dst() {
    mem_b temp;
    temp = header_bytes_received_at_dst_tor;
    header_bytes_received_at_dst_tor = 0; // reset the counter
    return temp;
}
