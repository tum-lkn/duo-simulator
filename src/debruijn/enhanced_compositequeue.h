...
#define QUEUE_ENC_RLB 1 // modified
...
class EnhancedCompositeQueue : public CompositeQueue {
 public:
    ...
    mem_b queuesize_high() override;

    mem_b queuesize_per_destination(uint32_t tor_id) override;

    mem_b queuesize_local_traffic() override;    // Additional statistics for debugging: Size in queue that originated in this racks
    mem_b queuesize_nonlocal_traffic() override;     // Additional statistics for debugging: Size in queue that originated in other racks

    void clear_queue() override;

    void strip_packets_after_da_disconnect() override { throw std::runtime_error("This EnhancedCompositeQueue does not support stripping packets after disconnect."); }
