...
NdpSrc::NdpSrc(DebruijnTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist,
               int flow_src, int flow_dst, bool vlb, bool is_priority_flow)
    ...
    _vlb = vlb;
    _is_priority_flow = is_priority_flow;
    ...
    */
    ...
    if (cum_ackno >= _flow_size){
	   //cout << "Flow " << nodename() << " finished at " << timeAsMs(eventlist().now()) << endl;
        ...
        cout << " nacks=" << _nacks_received;
        cout << " rtx_after_nack=" << _bytes_rtx_after_nack;
        cout << " rtx_received_pkts=" << _sink->_pkts_received_retransmits;
        cout << " rtx_received_bytes=" << _sink->_bytes_received_retransmits;

        cout  << " stripped=";
        for (uint i = 0; i < 10; i++) {
            cout << _stripped_at_hop[i] << ",";
        }
...
void NdpSrc::set_vlb_destination(NdpPacket *p) {
    int VLB_ATTEMPTS = 10;
    int srcrack = _top->get_firstToR(p->get_real_src());
    int dstrack = _top->get_firstToR(p->get_real_dst());
    for (int imrack_ind = 0; imrack_ind < VLB_ATTEMPTS; imrack_ind++) { // sweep NVLB possible intermediate ToRs
        int imrack = rand() % _top->no_of_tors(); // pick VLB intermediate ToRs randomly
        if (imrack != srcrack && imrack != dstrack) {
            // ATTENTION: We set here a rack as intermediate destination. real_src and real-dst are HOSTS!!!
            p->set_dst(imrack);
            break;
        }
    }
...
void NdpSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout <<"At " << timeAsUs(now) << "us RTO " << timeAsUs(_rto) << "us MDEV " << timeAsUs(_mdev) << "us RTT "<< timeAsUs(_rtt) << "us SEQ " << _last_acked / _mss << " CWND "<< _cwnd/_mss << " Flow ID " << _flow.flow_id()  << endl;
    /*
    if (_log_me) {
...
mem_b NdpSink::_global_received_bytes = 0;
...
NdpSink::NdpSink(DebruijnTopology* top, NdpPullPacer* pacer, int flow_src, int flow_dst)
    : Logged("ndp_sink"), _cumulative_ack(0) ,  _flow_src(flow_src), _flow_dst(flow_dst), _top(top),
    total_hops_per_bit(0), total_received_packets(0),
    _total_received(0)
{
    _src = 0;
    _pacer = pacer;
...
void NdpSink::receivePacket(Packet& pkt) {
    ...
    if (seqno == _cumulative_ack+1) { // it's the next expected seq no
        ...
        _top->reduce_received_demand(pkt.get_src(), pkt.get_dst(), size);
