...
class NdpSrc : public PacketSink, public EventSource {
 ...
 public:
    NdpSrc(DebruijnTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist,
           int flow_src, int flow_dst, bool vlb, bool is_priority_flow);

    NdpSrc(DebruijnTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist,
           int flow_src, int flow_dst, bool vlb) : NdpSrc(top, logger, pktlogger, eventlist,
                                                          flow_src, flow_dst, vlb, false) {};
    ...
    inline uint64_t get_flowsize() const {return _flow_size;} // bytes
    ...
    inline int get_flow_src() const {return _flow_src;}
    inline int get_flow_dst() const {return _flow_dst;}
    ...
    vector<uint32_t> _stripped_at_hop;
 ...
 private:
    ...
    void set_vlb_destination(NdpPacket *p);
};
...
class NdpSink : public PacketSink, public DataReceiver, public Logged {
 ...
 public:
    ...
    NdpSink(DebruijnTopology* top, NdpPullPacer* pacer, int flow_src, int flow_dst);
    ...
    uint32_t _drops;
    uint64_t cumulative_ack() override { return _cumulative_ack + _received.size()*9000;}
