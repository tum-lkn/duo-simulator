...
public:
    ...
    //
    inline static NdpPacket *newpkt(DebruijnTopology *top, PacketFlow &flow, int src, int dst, NdpSrc *ndpsrc,
                                    NdpSink *ndpsink, seq_t seqno, seq_t pacerno, int size, bool retransmitted,
                                    bool last_packet) {
        NdpPacket *p = _packetdb.allocPacket();
        p->set_attrs(flow, size + ACKSIZE, seqno + size - 1, src,
                     dst); // The NDP sequence number is the first byte of the packet; I will ID the packet by its last byte.
        p->set_topology(top);

        // Set real SRC and Dst in preparation for VLB
        p->set_real_src(src);
        p->set_real_dst(dst);
...
public:
    ...
    inline static NdpAck *newpkt(DebruijnTopology *top, PacketFlow &flow, int src, int dst,
...
public:
    ...
    inline static NdpNack *newpkt(DebruijnTopology *top, PacketFlow &flow, int src, int dst,
                                  NdpSrc *ndpsrc, seq_t pacerno, seq_t ackno, seq_t cumulative_ack, seq_t pullno) {
