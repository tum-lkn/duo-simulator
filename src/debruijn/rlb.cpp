...
RlbSrc::RlbSrc(DebruijnTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, int flow_src, int flow_dst)
    : EventSource(eventlist,"rlbsrc"),   _flow_src(flow_src), _flow_dst(flow_dst), _top(top), _logger(logger), _flow(pktlogger) {
...
void RlbSrc::sendToRlbModule() {
    RlbPacket* p = RlbPacket::newpkt(_top, _flow, _flow_src, _flow_dst,
                                     _sink, _mss, _pkts_sent);
...
RlbSink::RlbSink(DebruijnTopology* top, EventList &eventlist, int flow_src, int flow_dst)
...
void RlbSink::receivePacket(Packet& pkt) {
    ...
    if (_total_received >= _src->get_flowsize()) {
        ...
        cout << "FCT " << _src->get_flow_src() << " " << _src->get_flow_dst() << " " << _src->get_flowsize() <<
            " " << timeAsMs(eventlist().now() - _src->get_start_time()) <<
            " " << fixed << timeAsMs(_src->get_start_time()) << endl;
    }
}
