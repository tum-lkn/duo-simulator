...
Queue *ExpanderTopology::alloc_queue(QueueLogger *queueLogger, uint64_t speed, mem_b queuesize) {
    if (qt == RANDOM)
        ...
        return new EnhancedCompositeQueue(speedFromMbps(speed), queuesize, *eventlist, queueLogger);
    }
    assert(0);
}
            ...
            for (int imrack_ind = 0; imrack_ind < NVLB; imrack_ind++) { // sweep NVLB possible intermediate ToRs
                ...
                if (imrack != srcrack && imrack != destrack) {
                    ...
                    for (int i = 0; i < npaths1; i++) {
                        ...
                        for (int j = 0; j < nhops - 1; j++) {
                            routeout->push_back(
                                    queues_tor_tor[_rts[srcrack][imrack][i][j]][_rts[srcrack][imrack][i][j + 1]]);
                            routeout->push_back(
                                    pipes_tor_tor[_rts[srcrack][imrack][i][j]][_rts[srcrack][imrack][i][j + 1]]);
                        }
                        ...
                        for (int j = 0; j < nhops - 1; j++) {
                            routeout->push_back(
                                    queues_tor_tor[_rts[imrack][destrack][pathchoice][j]][_rts[imrack][destrack][pathchoice][
                                            j + 1]]);
                            routeout->push_back(
                                    pipes_tor_tor[_rts[imrack][destrack][pathchoice][j]][_rts[imrack][destrack][pathchoice][
                                            j + 1]]);
                        }
...
void UtilMonitor::printAggUtil() {
    ...
    for (int tor = 0; tor < _N; tor++) {
        ...
        for (int downlink = 0; downlink < _N; downlink++) {
            pipe = _top->get_pipe_tor(tor, downlink);
            if (pipe == nullptr) continue;
            uint64_t bytes = pipe->reportBytes();
            mystream << "Q" << downlink % _hpr << "=" << bytes << " ";
            pipe_util_header << "Q" << downlink % _hpr << "=" << pipe->reportHeaderBytes() << " ";
            pipe_avg_hop_count << "Q" << downlink % _hpr << "=" << pipe->reportAvgHopCount() << " ";
        }
        for (int downlink = 0; downlink < _hpr; downlink++) {
            pipe = _top->get_pipe_tor_serv(tor, downlink);
            if (pipe == nullptr) continue;
            uint64_t bytes = pipe->reportBytes();
            mystream << "Q" << _uplinks+downlink << "=" << bytes << " ";
            pipe_util_header << "Q" << _uplinks+downlink << "=" << pipe->reportHeaderBytes() << " ";
            pipe_avg_hop_count << "Q" << _uplinks+downlink << "=" << pipe->reportAvgHopCount() << " ";
    ...
    for (int tor = 0; tor < _N; tor++) {
        for (int node = 0; node < _hpr; node++) {
            Pipe *pipe = _top->get_pipe_serv_tor(tor, node);
            if (pipe == nullptr) continue;
            sh_pipe_data << "H" << tor*_hpr+node << "=" << pipe->reportBytes() << " ";
            sh_pipe_header << "H" << tor*_hpr+node << "=" << pipe->reportHeaderBytes() << " ";
        }
    }
    ...
    std::stringstream mystream_queues;
    Queue *queue;
    for (int tor = 0; tor < _N; tor++) {
        mystream_queues << "QT" << tor << " ";
         for (int downlink = 0; downlink < _N; downlink++) {
             queue = _top->get_queue_tor(tor, downlink);
             if (queue == nullptr) continue;
            uint64_t bytes = queue->queuesize();
            mystream_queues << "Q" << downlink << "=" << bytes << " ";
        }
         for (int downlink = 0; downlink < _hpr; downlink++) {
            queue = _top->get_queue_tor_serv(tor, downlink);
            if (queue == nullptr) continue;
            uint64_t bytes = queue->queuesize();
            mystream_queues << "Q" << downlink << "=" << bytes << " ";
        }
        for (int downlink = 0; downlink < _hpr; downlink++) {
            queue = _top->get_queue_serv_tor(tor, downlink);
            uint64_t bytes = queue->queuesize();
    ...
    cout << "GlobalReceivedBytes " << timeAsMs(eventlist().now()) << " " << TcpSink::get_global_received_bytes() << endl;
