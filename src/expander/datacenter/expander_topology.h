...
#ifndef QT
#define QT
typedef enum {RANDOM, COMPOSITE, ENHANCED_COMPOSITE} queue_type;
#endif
...
class ExpanderTopology: public Topology{
  ...
  Pipe *get_pipe_serv_tor(int tor, int host) { return pipes_serv_tor[tor*_ndl+host][tor]; }

  Queue *get_queue_serv_tor(int tor, int host) { return queues_serv_tor[tor*_ndl+host][tor]; }

  Pipe *get_pipe_tor(int tor, int port) { return pipes_tor_tor[tor][port]; }

  Pipe *get_pipe_tor_serv(int tor, int port) {return pipes_tor_serv.at(tor).at(tor*_ndl+port); }

  Queue *get_queue_tor(int tor, int port) { return queues_tor_tor.at(tor).at(port); }

  Queue *get_queue_tor_serv(int tor, int port) { return queues_tor_serv.at(tor).at(tor*_ndl+port); }
