...
TcpSrc::send_packets() {
    ...
    if (!_established) {
        ...
        Packet *p = TcpPacket::new_syn_pkt(_flow, *_route, 1, 1, this);
        _highest_sent = 1;
...
#else
        TcpPacket *p = TcpPacket::newpkt(_flow, *_route, _highest_sent + 1, data_seq,
                                         size_to_send, this);
#endif
...
#else
    TcpPacket *p = TcpPacket::newpkt(_flow, *_route, _last_acked + 1, data_seq, _mss, this);
#endif
...
TcpSink::TcpSink() : Logged("sink"), _cumulative_ack(0), _packets(0), _mSink(nullptr), _drops(0), _crt_path(0),
                     _src(nullptr), _route(nullptr) {
    _nodename = "tcpsink";
...
TcpSink::send_ack(simtime_picosec ts, bool marked) {
    ...
    TcpAck *ack = TcpAck::newpkt(_src->_flow, *rt, 0, _cumulative_ack,
                                 _mSink != nullptr ? _mSink->data_ack() : 0, _src);
