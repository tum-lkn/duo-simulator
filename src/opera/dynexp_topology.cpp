...
void DynExpTopology::set_params() {
    ...
    demand_matrix.resize(_ntor, vector<mem_b>(_ntor, 0));
}
...
void DynExpTopology::add_new_demand(uint src, uint dst, mem_b volume) {
    demand_matrix[get_firstToR(src)][get_firstToR(dst)] += volume;
}

void DynExpTopology::reduce_received_demand(uint src, uint dst, mem_b volume) {
    demand_matrix[get_firstToR(src)][get_firstToR(dst)] -= volume;
    if (demand_matrix[get_firstToR(src)][get_firstToR(dst)] < 0)
        demand_matrix[get_firstToR(src)][get_firstToR(dst)] = 0;
}