...
void SinkLoggerSampling::monitorSink(DataReceiver* sink){
    ...
    //_last_sndbuf.push_back(sink->sndbuf());
    _last_rate.push_back(0);
    //_multipath.push_back(0); // modification
}
...
void SinkLoggerSampling::doNextEvent(){
    ...
    _last_time = now;
    NdpAck::seq_t  deltaB; // modification (Tcp -> Ndp)
    uint32_t deltaSnd = 0;
    double rate;
...
{
    //cout << "NdpSinkLoggerSampling = " << timeAsSec(period) << " seconds, init \n";
}
...
void NdpSinkLoggerSampling::doNextEvent(){
    ...
    simtime_picosec delta = now - _last_time;
    _last_time = now;
    NdpAck::seq_t  deltaB; // modification (changed TcpAck -> NdpAck)
    uint32_t deltaSnd = 0;
    double rate;
