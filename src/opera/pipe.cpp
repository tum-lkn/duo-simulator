...
void Pipe::sendFromPipe(Packet *pkt) {
    ...
    //else {
        if (pkt->is_lasthop()) {
            ...
            // get the port:
            // TODO(johannes) Remove this, we don't know the path length in advance
            if (pkt->get_crthop() == pkt->get_maxhops()) { // no more hops defined, need to set downlink port
                ...
                // TODO provide here also the current location of the packet (instead of crthop)
...
void UtilMonitor::printAggUtil() {
    ...
    // debug: More detailed queue utilization
    cout << mystream.str();
    ...
    for (int tor = 0; tor < _N; tor++) {
        ...
        for (int downlink = 0; downlink < _hpr + _top->no_of_uplinks(); downlink++) {
            Queue *queue = _top->get_queue_tor(tor, downlink);
    ...
    std::stringstream demand_matrix;
    for (int tor = 0; tor < _N; tor++) {
        demand_matrix << "DM " << tor << " ";
        for (int dst = 0; dst < _N; dst++) {
            demand_matrix << _top->demand_matrix.at(tor).at(dst) << ",";
        }
        demand_matrix << endl;
    }

    cout << "Demand Matrix " << timeAsMs(eventlist().now()) << endl;
    cout << demand_matrix.str();

    cout << "GlobalReceivedBytes " << timeAsMs(eventlist().now()) << " "
        << RlbSink::get_global_received_bytes() + NdpSink::get_global_received_bytes() << endl;

    if (((int) timeAsMs(eventlist().now()) % 100) == 0) {
