...
loadfrac0=.8; % fraction of theoretically possible load

% ----- define simulation length (time):

totaltime=10.001; % seconds, probably want a little extra here...

filename=sprintf('flows_permutation_%.0fpercLoad_10sec_%dhosts',100*loadfrac0,H);

% NOTE: only inter-rack traffic allowed

% --- uniform
Nactivehosts=H;
Ncons=H; % number of possible connections
srcdst=zeros(Ncons,2);
cnt=0;
current_dest=2
# for a=randperm(Nrack) % sources
for a=1:Nrack
    if a~=current_dest
         % hosts are not in the same rack
        for h=1:Hosts_p_rack
            cnt=cnt+1;
            srcdst(cnt,:)=[(a-1)*Hosts_p_rack+h (current_dest-1)*Hosts_p_rack+h];
        end
        current_dest = current_dest + 1;
        if current_dest > Nrack
            current_dest = 1
        end
    end
end

srcdst=srcdst(randperm(Ncons), :);
...
flowsize=5000000; % bytes / flow
linkrate=10e9/8; % bytes / second

lambda_host_max=linkrate/flowsize; % flows / second (per host)
